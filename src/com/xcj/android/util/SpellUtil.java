...
public class SpellUtil {
	private static final String TAG = SpellUtil.class.getName();
	...
	private static SpellUtil sInstance;
	...
	protected SpellUtil(boolean hasChinaCollator) {
		mHasChinaCollator = hasChinaCollator;
	}

	public static SpellUtil getInstance() {
		synchronized(SpellUtil.class) {
			...
			for (int i = 0; i < locale.length; i++) {
				if (locale[i].equals(Locale.CHINA)) {
					sInstance = new SpellUtil(true);
					return sInstance;
				}
			}
			Log.w(TAG, "There is no Chinese collator, HanziToPinyin is disabled");
			sInstance = new SpellUtil(false);
			return sInstance;
		}
	}
	 ...
	 * @return
	 */
	public static String getFirstSpellLetter(String source) {
		ArrayList<Token> tokens = getInstance().get(source);
